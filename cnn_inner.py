...
def one_hot(Y):
	...
	for error in Y:
		...
		error = float(error)
		encoding = np.zeros(5,)    # array to hold one_hot encoding for 1 frame.

		encoding[int(error + 3)] = 1
...
def split(f,cmd):
	...
	img = cv2.imread("/home/fizzer/Pictures/inner_cmds/" + f)
...
def get_images(folder = '/home/fizzer/Pictures/inner_cmds/'):
...
#	conv_model.add(layers.MaxPooling1D((2)))
	...
	conv_model.add(layers.Conv1D(512, (3), activation='relu'))
	conv_model.add(layers.MaxPooling1D((2)))

	conv_model.add(layers.Flatten())
	conv_model.add(layers.Dropout(0.5))
	conv_model.add(layers.Dense(512, activation='relu'))
	conv_model.add(layers.Dense(5, activation='softmax'))
...
model.save("/home/fizzer/ros_ws/src/controller/src/Driver/driver_inner")
